...
import { getMemberOrderRepurchaseByIdAPI, getMemberOrderPreAPI, getMemberOrderPreNowAPI, postMemberOrderAPI } from '@/services/order';
...
const onOrderSubmit = async () => {
  ...
  const res = await postMemberOrderAPI({
    addressId: selecteAddress.value?.id, // 所选地址的id
    buyerMessage: buyerMessage.value, // 订单备注
    deliveryTimeType: activeDelivery.value.type, // 配送时间类型
    goods: orderPre.value!.goods.map((v) => ({ count: v.count, skuId: v.skuId })), // 商品集合
    payChannel: 2, // 支付渠道
